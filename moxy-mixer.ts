...
const mix = (js: string) => {
	...
	let topHeaders = `const ${cs[tokens.length + 1]}=decodeURIComponent,`
	topHeaders += `${cs[tokens.length + 2]}=document,`
	topHeaders += `${cs[tokens.length]}=(_s)=>_s[${
		cs[tokens.length + 1]
	...
	let times = 0
	const rngJs = ''
	const fn = cs[tokens.length]
	for (let i = 0; i < tokens.length; i++) {
		const rng = Math.floor(Math.random() * 3) + 1
		if (rng === 1) {
			times++
			const IDX = cs[tokens.length + 3 + times]
			let bytes: string = ''
			const bLength: number = Math.floor(Math.random() * 20) + 5
			const rngb = Math.floor(Math.random() * 3) + 1
			if (rngb === 1) {
				...
				headers += `${IDX}= '${bytes}',`
				rngJs += `const ___${cs[tokens.length + 1 + times]}${
					cs[tokens.length + 3 + times]
				}${cs[tokens.length + 2 + times]}__ = () => ${
					cs[tokens.length + 3 + times]
				}\n`
			} else if (rngb === 2) {
				for (let i = 0; i < bLength; i++) {
